...
@property (nonatomic, assign) FlutterViewController* flutterViewController;
...
- (void)clearChannels {
    ...
    FlutterMethodChannel *obj1 = (FlutterMethodChannel*)_navigationChannel.get();
    [obj1 destory];
    
    FlutterMethodChannel *obj2 = (FlutterMethodChannel*)_platformChannel.get();
    [obj2 destory];
    
    FlutterMethodChannel *obj3 = (FlutterMethodChannel*)_textInputChannel.get();
    [obj3 destory];
    
    //FlutterBasicMessageChannel
    FlutterBasicMessageChannel *m = (FlutterBasicMessageChannel*)_lifecycleChannel.get();
    [m destory];
    
    FlutterBasicMessageChannel *m1 = (FlutterBasicMessageChannel*)_systemChannel.get();
    [m1 destory];
    ...
    if (_regplugins) {
        [_regplugins enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, FlutterMethodChannel * obj, BOOL * _Nonnull stop) {
            if(obj) { [obj destory];  }
        }];
...
- (void)onUserSettingsChanged:(NSNotification*)notification {
    ...
    [_settingsChannel.get() sendMessage:@{
                                          @"textScaleFactor" : @([weak_flutter_view_controller textScaleFactor]),
                                          @"alwaysUse24HourFormat" : @([weak_flutter_view_controller isAlwaysUse24HourFormat]),
                                          }];
}
...
- (void)sendOnChannel:(NSString*)channel
    ...
    NSLog(@"sendOnChannel: %@ %@",channel,[[NSString alloc] initWithData:message encoding:NSUTF8StringEncoding]);
...
//这里注册插件，要持有，但是reg内部设置block打破循环引用
- (NSObject<FlutterPluginRegistrar>*)registrarForPlugin:(NSString*)pluginKey {
    NSAssert(self.pluginPublications[pluginKey] == nil, @"Duplicate plugin key: %@", pluginKey);
    self.pluginPublications[pluginKey] = [NSNull null];
    FlutterViewControllerRegistrar * reg = [[FlutterViewControllerRegistrar alloc] initWithPlugin:pluginKey
...
-(void)destory{
    ...
    if(_flutterViewController) { [_flutterViewController release]; _flutterViewController = nil; }
     if(_channel) { [_channel destory]; _channel = nil;}
}
- (void)dealloc {
    NSLog(@"registrarForPlugin dealloc: %@",self);
    [self destory];
    [super dealloc];
}
