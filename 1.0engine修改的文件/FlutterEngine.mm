...
//    }
    if (self.pluginPublications.count) {
        [self.pluginPublications enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            if(![obj isKindOfClass:NSNull.class] && [obj canPerformAction:@selector(destory) withSender:nil]) { [obj destory];  }
...
- (instancetype)initWithPlugin:(NSString*)pluginKey flutterEngine:(FlutterEngine*)flutterEngine {
  ...
  _pluginKey = [pluginKey retain];
    _flutterEngine = flutterEngine;//[flutterEngine getWeakPtr];// [flutterEngine retain];
  return self;
}
...
- (void)dealloc {
  ...
  NSLog(@"%s",__FUNCTION__);
  if(_pluginKey) [_pluginKey release];
